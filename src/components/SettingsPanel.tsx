    ...
    <div className="p-4 bg-white rounded-xl shadow-md">
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div className="col-span-2">
          <h3 className="font-semibold">Overskud pr. måned</h3>
          <div className="grid grid-cols-3 gap-2 mt-1">
            {months.map((m, i) => (
              <label key={m} className="flex flex-col">
                ...
                <input
                  type="number"
                  value={settings.monthlyProfits[i]}
                  onChange={(ev) => {
                    const arr = [...settings.monthlyProfits]
                    arr[i] = Number(ev.target.value)
                    update('monthlyProfits', arr)
                  }}
                />
              </label>
            ))}
