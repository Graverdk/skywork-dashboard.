...
function App() {
  const saved = load()
  const [settings, setSettings] = useState<Settings>(saved?.settings || defaultSettings)
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="p-4 bg-white rounded shadow space-y-2">
          ...
          <p>SJ pulje: {result.regions.SJ.pool.toFixed(0)}</p>
          <p>JY pulje: {result.regions.JY.pool.toFixed(0)}</p>
          ...
          <p>Udbetaling SJ: {result.regions.SJ.totalPayout.toFixed(0)}</p>
          <p>Udbetaling JY: {result.regions.JY.totalPayout.toFixed(0)}</p>
          <div className="space-x-2">
            <button onClick={exportData} className="px-2 py-1 bg-gray-200 rounded">
              Export JSON
            </button>
